...
async function starts() {
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Ja foi tarde kki @${num.split('@')[0]}👋`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				wait: 'Espera, to fazendo o bglh, cabeça de pica 👽',
				success: 'feito ma baby️',
				error: {
					stick: '❌ Falha, ocorreu um erro ao converter a imagem em um adesivo ❌',
					Iv: 'link errado parça'
				},
				only: {
					group: '❌ Este comando só pode ser usado em grupos! ❌',
					ownerG: '❌ Este comando só pode ser usado por. Este comando só pode ser usado pelo grupo proprietário! ❌',
					ownerB: '❌ Este comando só pode ser usado por administradores de grupo! ❌',
			...
			switch(command) {
				...
				case 'info':
					...
					teks = `*nome do bot* : ${me.name}\n*Nome do Bot* : @${me.jid.split('@')[0]}\n*Prefix* : ${prefix}\n*Todos contatos bloqueados* : ${blocked.length}\n*Tô vivo desde* : ${kyun(uptime)}`
					buffer = await getBuffer(me.imgUrl)
					client.sendMessage(from, buffer, image, {caption: teks, contextInfo:{mentionedJid: [me.jid]}})
					break
				case 'blocklist':
					teks = 'Todos contatos bloqueados :\n'
				...
				case 'tiktokstalk':
					try {
						...
						reply('Nome de usuário Provavelmente não válido')
					}
					break
				case 'nulis':
				case 'tulis':
				...
				case 'bc':
					...
					if (mentioned.length > 1) {
						teks = 'promovendo esse mendigo, so pedindo adm\n'
				...
				case 'listadmins':
					if (!isGroup) return reply(mess.only.group)
					teks = `todos os cornos *${groupMetadata.subject}*\nTotal : ${groupAdmins.length}\n\n`
					no = 0
					...
					break
                                ...
                                case 'leave':
                                        ...
                                        break
				case 'toimg':
					if (!isQuotedSticker) return reply('❌ o adesivo de resposta é um stick de resposta ❌')
					reply(mess.wait)
